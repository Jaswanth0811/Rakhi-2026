    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-[#16141D] border border-gold/20 rounded-3xl p-6 sm:p-8 space-y-6 shadow-xl text-center">
        ...
        <div className="space-y-2">
          ...
          <div className="font-mono text-4xl sm:text-5xl font-bold text-gold tracking-widest">
            {revealedCode || "••••••"}
          </div>
          <p className="text-xs text-goldlight/60">
            {revealedCode
        ...
        <button
          ...
          className="w-full py-4 rounded-xl bg-warmblack border border-gold/40 text-gold font-bold text-xs flex items-center justify-center gap-2 hover:bg-gold/10 transition-all cursor-pointer"
        >
          <RefreshCw className={`w-4 h-4 ${regenerating ? "animate-spin" : ""}`} />
          <span>{regenerating ? "GENERATING..." : "GENERATE NEW SECRET CODE"}</span>
        </button>

        <div className="pt-4 border-t border-gold/10 text-left space-y-3">
          <label className="block text-xs font-semibold text-goldlight/80">
            SHARE MESSAGE FOR {sister?.name.toUpperCase()}
          </label>
          <textarea
            rows={5}
            readOnly
            value={shareText}
            className="w-full p-4 rounded-xl bg-[#0E0D12] border border-gold/30 text-cream font-mono text-xs leading-relaxed resize-none focus:outline-none"
          />
          ...
          <button
            ...
            className="w-full py-3.5 rounded-xl bg-gradient-to-r from-gold via-goldlight to-golddark text-charcoal font-bold text-xs flex items-center justify-center gap-2 shadow-md cursor-pointer"
          >
            {copied ? (
              <>
